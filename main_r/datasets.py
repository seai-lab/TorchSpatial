    ...
    if params['dataset'] == 'inat_2017':
        ...
        if eval_split == 'val':
            ...
            val_split = pd.read_csv(
                data_dir + 'kaggle_solution_2017_DO_NOT_SHARE.csv')['usage'].values == 'Private'
        ...
        if load_cnn_predictions:
            val_preds = load_sparse_feats(
                data_dir + 'features_inception/inat2017_' + eval_split + '_preds_sparse.npz')

        if load_cnn_features:
            if params['inat2018_resolution'] == 'standard':
                val_feats = np.load(
                    data_dir + 'features_inception/inat2017_' + eval_split + '_net_feats.npy')
            elif params['inat2018_resolution'] == 'pretrain':
                val_feats = np.load(
                    data_dir + 'features_inception_pretrain/inat2017_' + eval_split + '_pretrain_net_feats.npy')
                print(
        ...
        if load_cnn_features_train:
            if params['inat2018_resolution'] == 'standard':
                ...
                train_feats = np.load(
                    data_dir + 'features_inception_pretrain/inat2017_train_pretrain_net_feats.npy')
                print(
        ...
        if eval_split == 'val':
            ...
            val_split = pd.read_csv(
                data_dir + 'kaggle_solution_2018_DO_NOT_SHARE.csv')['usage'].values == 'Private'
        if params["load_img"]:
        ...
        if load_cnn_predictions:
            if params['cnn_pred_type'] == "full":
                if params['inat2018_resolution'] == 'high_res':
                    val_preds = load_sparse_feats(
                        data_dir + 'features_inception_hr/inat2018_' + eval_split + '_preds_sparse.npz')
                else:
                    val_preds = load_sparse_feats(
                        data_dir + 'features_inception/inat2018_' + eval_split + '_preds_sparse.npz')
        ...
        if load_cnn_features:
            if params['inat2018_resolution'] == 'high_res':
                val_feats = np.load(
                    data_dir + 'features_inception_hr/inat2018_' + eval_split + '_net_feats.npy')
            elif params['inat2018_resolution'] == 'pretrain':
                if params['cnn_model'] == 'inception_v3':
                ...
                val_feats = np.load(
                    data_dir + f'{pretrain_folder}/inat2018_' + eval_split + '_pretrain_net_feats.npy')
                print(
                    f"Load Pretrained {params['cnn_model']} feature of inat_2018 {eval_split} data -> {val_feats.shape}")
            else:
                val_feats = np.load(
                    data_dir + 'features_inception/inat2018_' + eval_split + '_net_feats.npy')

        if load_cnn_features_train:
            if params['inat2018_resolution'] == 'high_res':
                train_feats = np.load(
                    data_dir + 'features_inception_hr/inat2018_train_net_feats.npy').astype(np.float32)
            elif params['inat2018_resolution'] == 'pretrain':
                if params['cnn_model'] == 'inception_v3':
                    pretrain_folder = "features_inception_pretrain/"
                else:
                    pretrain_folder = f"features_inception_pretrain-{params['cnn_model']}/"
                train_feats_file = data_dir + \
                ...
                train_feats = np.load(
                    data_dir + 'features_inception/inat2018_train_net_feats.npy').astype(np.float32)
        ...
        if load_cnn_features:
            if params['inat2018_resolution'] == 'pretrain':
                val_feats = np.load(data_dir + 'features_inception_pretrain/fmow_' +
                                    eval_split + '_pretrain_net_feats.npy').astype(np.float32)
                print(
        ...
        if load_cnn_features_train:
            if params['inat2018_resolution'] == 'pretrain':
                train_feats = np.load(
                    data_dir + 'features_inception_pretrain/fmow_train_pretrain_net_feats.npy').astype(np.float32)
                print(
        ...
        # load observations
        train_locs, train_classes, train_users, train_dates, train_inds = \
            load_bird_data(data_dir, ann_file_name, 'train',
                           train_remove_invalid, params['meta_type'])
        val_locs, val_classes, val_users, val_dates, val_inds = \
            load_bird_data(data_dir, ann_file_name, eval_split,
                           eval_remove_invalid, params['meta_type'])
        ...
        if load_cnn_predictions:
            ...
            val_preds = load_sparse_feats(
                data_dir + 'features_inception/birdsnap_' + eval_split + '_preds_sparse.npz')

        if load_cnn_features:
            val_feats = np.load(
                data_dir + 'features_inception/birdsnap_' + eval_split + '_net_feats.npy')
        ...
        if load_cnn_predictions:
            val_preds = load_sparse_feats(
                data_dir + 'features_inception/nabirds_' + eval_split + '_preds_sparse.npz')
        ...
        classes = dict(zip(list(range(num_classes)), [
                       str(i) for i in list(range(num_classes))]))
...
def load_inat_data(ip_dir, loc_file_name, ann_file_name, remove_empty=False, load_img=False):
    ...
    if load_img:
        return np.array(locs).astype(np.float32), np.array(classes).astype(np.int), \
            np.array(users).astype(np.int), np.array(dates).astype(
                np.float32), np.array(keep_inds), np.array(imgs)

    else:
        return np.array(locs).astype(np.float32), np.array(classes).astype(int), \
            np.array(users).astype(int), np.array(
                dates).astype(np.float32), np.array(keep_inds)
...
def load_yfcc_data(data_dir, ann_file_name, split_name):
    ...
    locs = da[da['split'] == split_name][[
        'lon', 'lat']].values.astype(np.float32)
