...
def unsupervise_train(model, data_loader, optimizer, epoch, params, logger = None, neg_rand_type='spherical'):
    model.train()
    ...
    for batch_idx, batch_data in enumerate(data_loader):
        ...
        if (batch_idx % params['log_frequency'] == 0 and batch_idx != 0) or (batch_idx == (len(data_loader)-1)):
            logger.info('[{}/{}]\tUnsupervised {} Loss  : {:.4f}'.format(batch_idx * params['batch_size'], 
...
def unsupervise_eval(model, data_loader, params, logger = None):
    ...
    logger.info('Unsupervised {} Test loss   : {:.4f}'.format(params["unsuper_loss"], loss_avg.avg))





def train(model, data_loader, optimizer, epoch, params, logger = None, neg_rand_type='spherical'):
    model.train()
    ...
    for batch_idx, batch_data in enumerate(data_loader):
        ...
        if (batch_idx % params['log_frequency'] == 0 and batch_idx != 0) or (batch_idx == (len(data_loader)-1)):
            logger.info('[{}/{}]\tLoss  : {:.4f}'.format(batch_idx * params['batch_size'], len(data_loader.dataset), loss_avg.avg))
